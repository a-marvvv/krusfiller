...
Func _UIA_ElementDoDefaultAction($oElement)
  If Not IsObj($oElement) Then Return SetError(1, __UIA_ConsoleWriteError("_UIA_ElementDoDefaultAction : параметр не является объектом"), 0)
  ...
  If Not IsObj($oIAccess) Then Return SetError(2, __UIA_ConsoleWriteError("_UIA_ElementDoDefaultAction : ошибка создания объекта на основе шаблона LegacyIAccessible"), 0)
  Local $iErrorCode = $oIAccess.DoDefaultAction()
  If $iErrorCode Then Return SetError(3, __UIA_ConsoleWriteError("_UIA_ElementDoDefaultAction : ошибка выполнения метода (0x" & Hex($iErrorCode) & ")"), 0)
  Return 1
...
;             :         @error = 4 - ошибка создания условия поиска (см. примечание о типизации)
;             :         @error = 5 - ошибка создания массива элементов (объектов)
; Автор       : InnI
...
Func _UIA_FindAllElements($oElementFrom, $vProperty = 0, $vPropertyValue = "")
  ...
  If @error Then Return SetError(2, __UIA_ConsoleWriteError("_UIA_FindAllElements : ошибка создания объекта UIAutomation"), 0)
  If Not $vProperty Then
    $oCondition = Default
  Else
    $vProperty = __UIA_GetPropIdFromStr($vProperty, "_UIA_FindAllElements")
    If $vProperty = -1 Then Return SetError(3, __UIA_ConsoleWriteError("_UIA_FindAllElements : ошибка преобразования свойства"), 0)
    $oCondition = _UIA_CreatePropertyCondition($vProperty, $vPropertyValue)
    If Not IsObj($oCondition) Then Return SetError(4, __UIA_ConsoleWriteError("_UIA_FindAllElements : ошибка создания условия поиска (см. примечание о типизации)"), 0)
  EndIf
  Local $aElements = _UIA_FindAllElementsEx($oElementFrom, $oCondition)
  If Not IsArray($aElements) Then Return SetError(5, __UIA_ConsoleWriteError("_UIA_FindAllElements : ошибка создания массива элементов (объектов)"), 0)
  Return $aElements
